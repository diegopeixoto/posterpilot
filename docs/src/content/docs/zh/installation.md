...
- **`/data`** — 持久状态：SQLite、设置、加密密钥、海报快照／修订、
  应用备份、缓存和轮转日志（`/data/logs/posterpilot.log`）。
...
派生密钥。当你希望在容器（及其 `data/.app-key`）被重建时密钥仍保持
可移植时，请设置它。如果你不
...
等待修改任务完成并复制整个 `/data` 卷。如果当前版本已有**设置 → 备份与恢复**，
再创建并验证一个手动备份。升级后必须保留相同的 `APP_SECRET` 或 `.app-key`。

升级会运行增量数据库迁移。单服务器安装会迁移为受保护的命名 **Default server**，
不会丢弃项目或历史。请遵循[多服务器迁移检查表](../multi-server-migration/)。
...
2. 首次运行时还没有同步任何内容。一个横幅会指向位于 `/setup`
   ...
   你也可以改为在 **设置** 中配置所有内容。每一步只有服务器接受后才前进；
   最后一步会跟踪首次同步到终态，并在需要时显示失败详情和重试。
...
请使用**设置 → 备份与恢复**，不要手工替换运行中的 SQLite。预览会验证校验和、
数据库、架构、空间、路径和密钥。确认会进入维护模式、等待修改任务完成、创建安全
备份并准备标记。重启容器，在 libsql 打开前替换，然后检查就绪报告。参阅
[自动化与恢复](../automation-recovery/)。
