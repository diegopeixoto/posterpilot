...
`TMDB_ARTWORK_LANGUAGE`（默认 `any`）决定浏览和自动选择哪些语言的 TMDB 图片，且与 `APP_LANGUAGE` 相互独立：`any` 保留 TMDB 返回的全部语言；`ui` 跟随界面语言并归一化为基础代码（`pt-BR` 界面偏好 `pt`）；也可以直接写 ISO 639-1 基础代码（`en`、`de` 等），不限于已翻译的六种界面语言。无法识别的值按未设置处理并回退到 `any`，而不会套用错误的筛选。与其他设置一样，环境变量优先，界面中该字段显示为环境管理。

TMDB 未标注语言的图片视为中性图片，在任何偏好下都保留，因此偏好绝不会清空只有中性图片的面板。发现始终保留所有语言——偏好只影响浏览与自动选择，不影响下载内容——所以更改偏好只是重新筛选已有候选，无需重新搜索。仅当没有偏好语言且没有未标注的候选时，自动选择才会退回其他语言的海报，并在这种情况下加以标注。项目页另有临时的**偏好／全部**切换，可在单个标题上越过偏好查看，而不改动全局设置。

## 候选清单与“加载更多”

TMDB 采集过去**按图片类型**各截取 20 张——海报与背景图分别计数，这正是“上限 40 张封面”反馈的由来。现在会保留远多于此的候选，按 TMDB 自身的文件标识去重并保持 TMDB 的排序，每个提供方／图片类型的面板先显示有限的一批，并提供**加载更多**控件说明还有多少候选未显示。海报面板与背景图面板各自独立展开。发现仍保留**每种图片类型 200 个候选**的防御性上限——这是存储与渲染的边界，而非质量筛选：面板达到该上限时会提示清单已**截断**，而不是暗示这已是 TMDB 的全部内容。
