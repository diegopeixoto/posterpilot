...
:::caution[临时]
Jellyfin 支持是在共享的媒体服务器接口之后提供的。请针对你的
配置验证具体细节；下面的字段形态与配置
模型一致，但该集成比 Plex 路径更新。
:::

Jellyfin 需要一个基础 URL（`JELLYFIN_URL`）和一个 API 密钥（`JELLYFIN_API_KEY`）。
设置 `SERVER_TYPE=jellyfin` 使其成为活动服务器。海报会被上传到
Jellyfin 图像 API（海报用 `Primary`，背景用 `Backdrop`）。

### Emby

:::caution[临时]
Emby 支持与 Jellyfin 一样，是在共享的媒体服务器接口之后提供的，
并且比 Plex 路径更新。请针对你的配置验证具体细节。
:::

Emby 需要一个基础 URL（`EMBY_URL`）和一个 API 密钥（`EMBY_API_KEY`）。设置
`SERVER_TYPE=emby` 使其成为活动服务器。
...
关于导出如何被使用，参见 [使用](/posterpilot/zh/usage/)。
...
:::note
`DATABASE_URL` 和 `PORT` 是部署级设置——它们仅从
环境中读取，不属于应用内的设置页面。
:::
