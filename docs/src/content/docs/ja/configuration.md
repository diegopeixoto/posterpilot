...
:::caution[暫定]
Jellyfin サポートは、共有のメディアサーバーインターフェースの背後で提供されています。
詳細についてはあなたのセットアップに対して検証してください。以下のフィールド形状は設定モデルに
一致しますが、この統合は Plex の経路よりも新しいものです。
:::

Jellyfin にはベース URL（`JELLYFIN_URL`）と API キー（`JELLYFIN_API_KEY`）が必要です。
アクティブなサーバーにするには `SERVER_TYPE=jellyfin` を設定します。ポスターは Jellyfin の
画像 API（ポスターには `Primary`、背景には `Backdrop`）にアップロードされます。

### Emby

:::caution[暫定]
Emby サポートも Jellyfin と同様に、共有のメディアサーバーインターフェースの背後で提供され、
Plex の経路よりも新しいものです。詳細についてはあなたのセットアップに対して検証してください。
:::

Emby にはベース URL（`EMBY_URL`）と API キー（`EMBY_API_KEY`）が必要です。アクティブな
サーバーにするには `SERVER_TYPE=emby` を設定します。
...
:::note
`DATABASE_URL` と `PORT` はデプロイレベルの設定です — これらは環境からのみ読み取られ、
アプリ内の設定ページには含まれません。
:::
