...
:::caution[Provisional]
El soporte para Jellyfin se proporciona detrás de la interfaz compartida de
servidor multimedia. Valida los detalles con tu configuración; las formas de campo
de abajo coinciden con el modelo de configuración, pero la integración es más nueva
que la ruta de Plex.
:::
...
activo. Los pósters se suben a la API de imágenes de Jellyfin (`Primary` para el
póster, `Backdrop` para el fondo).

### Emby

:::caution[Provisional]
El soporte para Emby, como el de Jellyfin, se proporciona detrás de la interfaz
compartida de servidor multimedia y es más nuevo que la ruta de Plex. Valida los
detalles con tu configuración.
:::

Emby necesita una URL base (`EMBY_URL`) y una clave de API (`EMBY_API_KEY`). Define
`SERVER_TYPE=emby` para convertirlo en el servidor activo.
...
:::note
`DATABASE_URL` y `PORT` son ajustes a nivel de despliegue: se leen solo del entorno
y no forman parte de la página de Ajustes de la app.
:::
</content>
